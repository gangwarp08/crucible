// Deterministic synthetic dataset generator for the fde-api-integration-pro
// scenario (family 2 hard-band variant, P3.1 — seeded DORMANT by 0023).
//
// Same radical bug class as the canonical mid scenario PLUS a real secondary
// issue: a token-rotation race makes a couple of 401 retries fail too, so a
// small slice of the gap genuinely IS auth — the teammate's "it's just the
// tokens" theory is partially true and must be quantified, not hand-waved
// away, before it can be de-prioritized against the dominant cursor bug.
//
...
//                         root_cause }
//
// Run: pnpm exec tsx fixtures/fde-api-integration-pro/generate.ts
//
...
const SEED_LABEL = "fde-api-integration-pro-v1";
const COMPANY = "Atlas Freight";
const PROVIDER = "ContactHub";
const TARGET_CONTACTS = 7400;   // provider-side contact count (± jitter)
const PAGE_SIZE = 40;           // provider page size for /v2/contacts
const SKIP_INCIDENT_COUNT = 9;  // stale-cursor retries → pages silently skipped
const AUTH_401_COUNT = 6;       // expired-token 401s (red herring — all retried OK)
const AUTH_DROP_COUNT = 2;      // pro-only: 401s whose retry ALSO fails (pages lost)
